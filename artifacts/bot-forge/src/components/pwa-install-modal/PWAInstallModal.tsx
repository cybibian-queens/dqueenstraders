...
const PWAInstallModal: React.FC = () => {
    ...
    const getInstallSteps = () => {
        if (isDesktop) {
            return [
                ...
                localize('Follow the prompts to install DQueens Traders.'),
            ];
        } else if (isIOS) {
            return [
                localize('Open DQueens Traders in your browser.'),
        ...
        <div className='pwa-install-modal__content'>
            <div className='pwa-install-modal__description'>
                <Text size='xs' color='prominent'>
                    {isPWALaunch
                        ? localize("Welcome to DQueens Traders! You're using the mobile app version.")
                        : isDesktop
                          ? localize('Install DQueens Traders as a desktop app for faster access and a native experience!')
                          : localize(
                                "We're excited to announce that DQueens Traders platform is now a Progressive Web App (PWA)!"
                            )}
                </Text>
            </div>
    ...
    const modalContent = isMobile ? (
        <MobileFullPageModal
            ...
            page_header_text={localize('Install DQueens Traders')}
        ...
        <Modal
            ...
            title={isDesktop ? localize('Install DQueens Traders') : localize('Introducing our installable app')}
            has_close_icon={true}
